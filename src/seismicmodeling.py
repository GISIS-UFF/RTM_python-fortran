...
if N_shot == 1:
    ...
    for shot in np.arange(0,N_shot):
        proc = mp.Process(target=aux.modelagemparalela, args=(shot+1,Fx[shot],Fz[shot],fonte,regTTM,parametro.modeloreal,parametro.sismogramaobservado,parametro.nome_prin))
        procs.append(proc)
        proc.start()
